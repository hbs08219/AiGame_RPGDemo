...
(() => {
  ...
  const createActionPlacement = config.createActionPlacement || 'detail-actions';
  ...
  root.innerHTML = `<header><h1></h1><select id="resource-switch" class="resource-switch" aria-label="数据资源"></select></header><main class="shell"><aside><div class="list-header"><input id="search" class="search" type="search" placeholder="搜索…" aria-label="搜索">${listCreateControl}</div><div id="records" class="list"><div class="empty">正在加载…</div></div></aside><section class="detail" aria-label="详情编辑"><h2 id="detail-title"></h2><form id="editor-form"></form><div class="actions"><button type="button" data-crud-operation="read">刷新</button>${detailCreateControl}<button type="button" data-crud-operation="update">保存修改</button><button class="danger" type="button" data-crud-operation="delete">删除</button></div><div id="output" class="status" role="status" aria-live="polite">尚未选择记录，可填写表单后新建。</div></section></main>`;
