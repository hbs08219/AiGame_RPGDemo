...
(() => {
  ...
  function renderList() {
    ...
    if (!shown.length) {
      ...
      empty.textContent = items.length ? '没有匹配记录' : '暂无数据，可在右侧新建';
    ...
    for (const item of shown) {
      ...
      control.textContent = String(record?.[resource.idField] || item?.id || '未命名记录');
